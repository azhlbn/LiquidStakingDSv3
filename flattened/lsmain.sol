...
contract LiquidStakingMain is AccessControlUpgradeable, LiquidStakingStorage {
    ...
    function stake(
        ...
        for (uint256 i; i < l; i++) {
            if (dapps[_utilities[i]].stakers[msg.sender].lastClaimedEra == 0)
                dapps[_utilities[i]].stakers[msg.sender].lastClaimedEra = _era + 1; // prettier-ignore
            ...
            if (_amounts[i] > 0) {
                ...
                // ds v3 update
                _updateSubperiodStakes(int256(amount));

                DAPPS_STAKING.lock(uint128(amount));
    ...
    function unstake(
        ...
        for (uint256 i; i < l; i++) {
            ...
            if (_amounts[i] > 0) {
                ...
                // ds v3 update
                _updateSubperiodStakes(~int256(_amount) + 1); // convert arg to negative number
    ...
    function updates(uint256 _era) internal {
    ...
    /// @param _era => latest era to update
    function _globalUnstakeForUtility(string memory _utility, uint256 _era) internal {
        Dapp storage dapp = dapps[_utility];
    ...
    function _periodUpdate() internal {
        ...
        if (!period.initiated) return;

        period.initiated = true;
    ...
    function _calcUserRewards(
        ...
        for (uint256 i = user.lastClaimedEra; i < lastUpdated; i = _uncheckedIncr(i)) {
            if (userEraBalance > 0) {
                ...
                userData[1] +=
                    (userEraRewards * (100 - userEraFee - UNSTAKING_FEE)) /
                    100;
            }
